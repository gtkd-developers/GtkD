...
extern(C)
{
	static GType gtk_menu_get_type();
	static GtkWidget * gtk_menu_new();

/* Display the menu onscreen */
	 static void gtk_menu_popup(	GtkWidget * menu,	GtkWidget * parent_menu_shell,	GtkWidget * parent_menu_item,	void * func,	gpointer data,	guint button,	guint32 activate_time);

/* Position the menu according to its position function. Called
 * from gtkmenuitem.c when a menu-item changes its allocation
 */
	static void gtk_menu_reposition(GtkWidget * menu);

	static void gtk_menu_popdown(GtkWidget * menu);

/* Keep track of the last menu item selected. (For the purposes
 * of the option menu
 */
	static GtkWidget * gtk_menu_get_active(GtkWidget * menu);
	static void gtk_menu_set_active(GtkWidget * menu, guint index_);
 ...
 */
	static void gtk_menu_set_accel_group(GtkWidget * menu, /*GtkAccelGroup*/ void * accel_group);
	static  GtkAccelGroup * gtk_menu_get_accel_group(GtkWidget * menu);
	static void gtk_menu_set_accel_path(GtkWidget * menu, char * accel_path);

/* A reference count is kept for a widget when it is attached to
 * a particular widget. This is typically a menu item; it may also
 * be a widget with a popup menu - for instance, the Notebook widget.
 */
	static void gtk_menu_attach_to_widget(GtkWidget * menu, GtkWidget * attach_widget, /*GtkMenuDetachFunc*/ void * detacher);
	static void gtk_menu_detach(GtkWidget * menu);
 ...
 */
	...
	static void gtk_menu_set_tearoff_state(GtkWidget * menu, gboolean torn_off);
	static gboolean gtk_menu_get_tearoff_state(GtkWidget * menu);

/* This sets the window manager title for the window that
 * appears when a menu is torn off
 */
	static void gtk_menu_set_title(GtkWidget * menu, char * title);
	static char * gtk_menu_get_title(GtkWidget * menu);

	static void gtk_menu_reorder_child(GtkWidget * menu, GtkWidget * child, gint position);
	
	void gtk_menu_set_screen(GtkWidget * menu, GdkScreen * screen);
}
