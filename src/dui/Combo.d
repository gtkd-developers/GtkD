...
extern (C)
{
	...
	void gtk_combo_set_value_in_list (void* combo, int val, int ok_if_empty);
	/* set/unset arrows working for changing the value (can be annoying) */
	void gtk_combo_set_use_arrows (void* combo, int val);
	/* up/down arrows change value if current value not in list */
	void gtk_combo_set_use_arrows_always (void* combo, int val);
	/* perform case-sensitive compares */
	void gtk_combo_set_case_sensitive (void* combo, int val);
	/* call this function on an item if it isn't a label or you
	 want it to have a different value to be displayed in the entry */
	void gtk_combo_set_item_string (void* combo, void* item,char* item_value);
	/* simple interface */
	void gtk_combo_set_popdown_strings (void* combo, void *strings);

	void gtk_combo_disable_activate (void* combo);
}
