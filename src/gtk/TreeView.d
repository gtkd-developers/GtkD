...
public class TreeView : Container
{
	...
	public int getPathAtPos(int x, int y, inout TreePath path, inout TreeViewColumn column, out int cellX, out int cellY)
	{
		...
		GtkTreePath* gtktreepath (path is null) ? null : path.getinout TreePathStruct();
		GtkTreeViewColumn* gtktreeviewcolumn (column is null) ? null : column.getinout TreeViewColumnStruct();
		
		auto p = gtk_tree_view_get_path_at_pos(gtkTreeView, x, y, &gtktreepath, &gtktreeviewcolumn, &cellX, &cellY);
		
		path = new inout TreePath(gtktreepath);
		column = new inout TreeViewColumn(gtktreeviewcolumn);
		return p;
	}
