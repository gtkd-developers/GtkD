	...
	public this (GtkMenu* gtkMenu)
	{
		version(noAssert)
		{
			if ( gtkMenu is null )
			{
				...
				version(Tango)
				{
					Stdout("struct gtkMenu is null on constructor").newline;
				}
				else
				{
					printf("struct gtkMenu is null on constructor");
				}
				zero = zero / zero;
			}
		}
		...
		{
			assert(gtkMenu !is null, "struct gtkMenu is null on constructor");
		}
