...
private import gdkpixbuf.Pixbuf;private import gdkpixbuf.PixbufFormat;
/**
 * Description
...
public class Pixbuf
{
	...
	public this (GdkColorspace colorspace, int hasAlpha, int bitsPerSample, int width, int height)
	{
		...
		this(cast(GdkPixbuf*)gdk_pixbuf_new(colorspace, hasAlpha, bitsPerSample, width, height) );
	}
	 ...
	 *  GdkPixbufLoader.
	 */
	public static PixbufFormat getFileInfo(char[] filename, int* width, int* height)
	{
		// GdkPixbufFormat* gdk_pixbuf_get_file_info (const gchar *filename,  gint *width,  gint *height);
		return new PixbufFormat( gdk_pixbuf_get_file_info(std.string.toStringz(filename), width, height) );
	}
