...
__gshared extern(C)
{
	...
	int g_bookmark_file_load_from_data(GBookmarkFile* bookmark, const(char)* data, size_t length, GError** err);
	...
	GVariant* g_variant_new_byte(char value);
	...
	char g_variant_get_byte(GVariant* value);
	...
	int g_spawn_command_line_async(const(char)* commandLine, GError** err);
	int g_spawn_command_line_sync(const(char)* commandLine, char** standardOutput, char** standardError, int* exitStatus, GError** err);
	...
	int g_snprintf(char* string_, gulong n, char* format, ... );
	...
	int g_vasprintf(char** string_, char* format, void* args);
	int g_vprintf(char* format, void* args);
	int g_vsnprintf(char* string_, gulong n, char* format, void* args);
	int g_vsprintf(char* string_, char* format, void* args);
	int g_fprintf(FILE* file, char* format, ... );
	int g_vfprintf(FILE* file, char* format, void* args);
	...
	char** g_environ_setenv(char** envp, const(char)* variable, const(char)* value, int overwrite);
	...
	char* g_convert(const(char)* str, ptrdiff_t len, const(char)* toCodeset, const(char)* fromCodeset, size_t* bytesRead, size_t* bytesWritten, GError** err);
	GQuark g_convert_error_quark();
	char* g_convert_with_fallback(const(char)* str, ptrdiff_t len, const(char)* toCodeset, const(char)* fromCodeset, const(char)* fallback, size_t* bytesRead, size_t* bytesWritten, GError** err);
	char* g_convert_with_iconv(const(char)* str, ptrdiff_t len, GIConv converter, size_t* bytesRead, size_t* bytesWritten, GError** err);
	...
	char* g_locale_to_utf8(const(char)* opsysstring, ptrdiff_t len, size_t* bytesRead, size_t* bytesWritten, GError** err);
	...
	int g_shell_parse_argv(const(char)* commandLine, int* argcp, char*** argvp, GError** err);
	char* g_shell_quote(const(char)* unquotedString);
	char* g_shell_unquote(const(char)* quotedString, GError** err);
