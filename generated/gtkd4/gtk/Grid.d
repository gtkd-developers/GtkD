...
public class Grid : Container, OrientableIF
{
	 ...
	 *     left = the column used to attach the left side of @child
	 *     top = the row used to attach the top side of @child
	 *     width = the number of columns @child spans
	 *     height = the number of rows @child spans
	 */
	public void queryChild(Widget child, out int left, out int top, out int width, out int height)
	{
		gtk_grid_query_child(gtkGrid, (child is null) ? null : child.getWidgetStruct(), &left, &top, &width, &height);
	}
