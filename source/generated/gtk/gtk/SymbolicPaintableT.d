 ...
 * Since: 4.6
 */
public template SymbolicPaintableT(TStruct)
{
	...
	public GtkSymbolicPaintable* getSymbolicPaintableStruct(bool transferOwnership = false)
	{
		if (transferOwnership)
			ownedRef = false;
		return cast(GtkSymbolicPaintable*)getStruct();
	}
	...
	public void snapshotSymbolic(Snapshot snapshot, double width, double height, RGBA[] colors)
	{
		...
		gtk_symbolic_paintable_snapshot_symbolic(getSymbolicPaintableStruct(), (snapshot is null) ? null : snapshot.getSnapshotStruct(), width, height, colorsArray.ptr, cast(size_t)colors.length);
	}
}
