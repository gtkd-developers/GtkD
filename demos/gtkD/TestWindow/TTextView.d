...
//	}
	...
	void setupWidgets()
	{
		auto vPaned = new Paned(GtkOrientation.VERTICAL);
	...
	void createTags(TextBuffer buffer)
	{
		...
		*/
		...
		auto headingTag = new TextTag("heading");
		headingTag.setProperty("weight", PangoWeight.BOLD);
		headingTag.setProperty("size", 15*PANGO_SCALE);
		tagTable.add(headingTag);
	...
	void insertText(TextBuffer buffer)
	{
		...
		TextIter start;
		TextIter end;
		// auto start = buffer.getStartIter();
		// auto iter = buffer.getStartIter();
		// auto end = buffer.getEndIter();
		Pixbuf pixbuf;
		Pixbuf scaled;
	...
	void attachWidgets(TextView view)
	{
		...
		while (findAnchor(iter))
		{
			...
			{
				auto hScale = new Scale(GtkOrientation.HORIZONTAL, 0.0, 100.0, 5.0);
