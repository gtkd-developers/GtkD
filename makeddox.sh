...
grep -h -e "^module" srcgstreamer/* -r | sort -u | sed 's/;//' | sed 's/\r//' |  sed 's/module \(.*\)$/\t$(MODULE \1)/' >> modules.ddoc
grep -h -e "^module" srcpeas/* -r | sort -u | sed 's/;//' | sed 's/\r//' |  sed 's/module \(.*\)$/\t$(MODULE \1)/' >> modules.ddoc
...
dmd -o- -D -X -Xfddox/docs.json -Ddddox/temp modules.ddoc docs/candy.ddoc \
	...
	srcvte/vtec/* srcvte/vte/* \
	srcgstreamer/gstreamer/* srcgstreamer/gstinterfaces/* srcgstreamer/gstreamerc/* \
	srcpeas/peas/* srcpeas/peasc/* -op
