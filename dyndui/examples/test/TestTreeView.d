 ...
 * }<br>
 */
class TestTreeView : ScrolledWindow , TreeViewListener
{
	Image image;		
	Pixbuf pixbuf;
	...
	enum columns
	{
		...
		duiObject = DUIType.STRING,
		gtkObject = DUIType.STRING,
		desc = DUIType.STRING,
		percentDone = DUIType.STRING,
		author = DUIType.STRING
	}
		...
		void* getNodeValue(int column)
		{
			...
			switch (column)
			{
				...
				case 3: 
					switch(percent)
					{
						case -1: value = (new String("uggly: won't get in")).toStringz(); break;
						case 0: value = (new String("unknown")).toStringz(); break;
						case 1: value = (new String("not yet")).toStringz(); break;
						case 98: value = (new String("Almost")).toStringz(); break;
						case 99: value = (new String("AGAIG")).toStringz(); break;
						case 100: value = (new String("Complete")).toStringz(); break;
						default:value = (new String(percent) ~ "%").toStringz(); break;
					}
					break;
					
				case 4: value = author; break;
				default: value = (new String("N/A")).toStringz(); break;
			}
			return value;
		}
	}
	...
	TreeView setup()
	{
		...
		column = new TreeViewColumn("Description",new CellRendererText(),"text", 2);
		treeView.appendColumn(column);
		column.setResizable(true);
		column.setReorderable(true);
		...
		column = new TreeViewColumn("Status",new CellRendererText(),"text", 3);
		treeView.appendColumn(column);
		column.setResizable(true);
		column.setReorderable(true);
		...
		testTreeStore.set(peekIter(true), new HTreeNode("DUIObject","GtkObject","The main gtk object",100));
