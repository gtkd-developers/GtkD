...
static char gray50_bits[] = [0x02, 0x01];
		...
		buffer.createTag("center",
			      "justification", cast(int)Justify.CENTER);

		buffer.createTag("right_justify",
			      "justification", cast(int)Justify.RIGHT);
	...
	void attachWidgets(TextView view)
	{
		...
		while (findAnchor(iter))
		{
			...
			else if (i == 1)
        	{
				...
				menu.append(new MenuItem("Option1"));
				menu.append(new MenuItem("Option2"));
				menu.append(new MenuItem("Option3"));
